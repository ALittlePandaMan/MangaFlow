...
export function EditorSkeleton() {
  return <div className="loading-skeleton-page grid h-full min-h-[560px] grid-rows-[44px_minmax(0,1fr)] bg-canvas" aria-busy="true" aria-label="正在打开工作台"><div className="flex items-center gap-3 border-b border-line-subtle bg-panel px-4">{[0,1,2,3,4,5].map(index => <SkeletonBlock className="size-7" key={index}/>)}</div><div className="grid min-h-0 grid-cols-[192px_minmax(0,1fr)_320px]"><aside className="border-r border-line-subtle bg-panel p-3"><SkeletonBlock className="h-3 w-16"/>{[0,1,2].map(item => <div className="mt-4" key={item}><SkeletonBlock className="aspect-[3/4] w-full"/><SkeletonBlock className="mt-2 h-2.5 w-4/5"/><SkeletonBlock className="mt-2 h-2 w-2/5"/></div>)}</aside><main className="relative grid place-items-center bg-[linear-gradient(45deg,var(--color-panel)_25%,transparent_25%),linear-gradient(-45deg,var(--color-panel)_25%,transparent_25%),linear-gradient(45deg,transparent_75%,var(--color-panel)_75%),linear-gradient(-45deg,transparent_75%,var(--color-panel)_75%)] [background-position:0_0,0_10px,10px_-10px,-10px_0] [background-size:20px_20px]"><div className="w-[min(420px,60%)]"><SkeletonBlock className="aspect-[3/4] w-full rounded-none"/><div className="mt-4 flex justify-center gap-2"><SkeletonBlock className="h-2 w-16"/><SkeletonBlock className="h-2 w-10"/></div></div></main><aside className="border-l border-line-subtle bg-panel p-5"><SkeletonBlock className="h-4 w-24"/>{[0,1,2,3,4,5].map(item => <div className="mt-5" key={item}><SkeletonBlock className="h-2.5 w-20"/><SkeletonBlock className="mt-2 h-10 w-full"/></div>)}</aside></div></div>
}
