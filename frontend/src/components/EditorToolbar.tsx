import { BoxSelect, Brush, Eraser, Focus, LassoSelect, MousePointer2, PenTool, Redo2, Scan, Undo2, ZoomIn, ZoomOut } from 'lucide-react'
...
const tools: {id: Tool, title: string, shortcut: string, icon: React.ReactNode}[] = [
  ...
  {id: 'polygon', title: '多边形区域（双击完成）', shortcut: 'P', icon: <PenTool size={17}/>},
...
export function EditorToolbar({ onUndo, onRedo, canUndo, canRedo, disabled = false }: {onUndo: () => void, onRedo: () => void, canUndo: boolean, canRedo: boolean, disabled?: boolean}) {
  ...
  return <div className="flex h-11 items-center border-b border-line bg-surface px-3">
    <div className="flex gap-2">{tools.map(item => <button disabled={disabled} key={item.id} className={cn(toolButton, tool === item.id && '!border-accent !bg-accent !text-[#06241c] shadow-[0_0_0_2px_rgb(16_211_163/.12)] hover:!bg-accent-hover hover:!text-[#06241c]')} title={`${item.title} (${item.shortcut})`} aria-label={item.title} aria-keyshortcuts={item.shortcut} aria-pressed={tool === item.id} onClick={() => setTool(item.id)}>{item.icon}</button>)}</div>
