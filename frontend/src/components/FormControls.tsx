...
export function SelectControl({value, options, onChange, disabled = false, name, placeholder = '请选择', ariaLabel, compact = false}: SelectControlProps) {
  ...
  return <div className={cn('relative w-full min-w-0', disabled && 'opacity-50')}>
    ...
    <button ref={triggerRef} type="button" className={cn('flex w-full cursor-pointer items-center justify-between gap-2 rounded-lg border bg-canvas text-left text-ink outline-none transition duration-150 hover:border-line-strong hover:bg-panel focus-visible:ring-3 focus-visible:ring-accent/15 disabled:cursor-not-allowed', compact ? 'h-[34px] px-2 text-[11px]' : 'h-[38px] px-3 text-xs', open ? 'border-accent shadow-[0_0_0_3px_rgb(16_211_163/.1)]' : 'border-line')} disabled={disabled} aria-label={ariaLabel} aria-haspopup="listbox" aria-expanded={open} aria-controls={menuId} onClick={() => setOpen(current => !current)} onKeyDown={onKeyDown}>
...
export function NumberControl({value, defaultValue = '', onChange, name, min, max, step = 1, disabled = false, placeholder, ariaLabel, compact = false, mixed = false, onStep}: NumberControlProps) {
  ...
  return <div className={cn('grid min-w-0 overflow-hidden rounded-lg border border-line bg-canvas transition hover:border-line-strong hover:bg-panel focus-within:border-accent focus-within:ring-3 focus-within:ring-accent/15', compact ? 'h-[34px] grid-cols-[minmax(0,1fr)_24px]' : 'h-[38px] grid-cols-[minmax(0,1fr)_30px]', disabled && 'opacity-50')}>
...
export function ColorControl({value, onChange, disabled = false, ariaLabel, compact = false}: ColorControlProps) {
  ...
  return <div className="relative w-full min-w-0">
    <button ref={triggerRef} type="button" className={cn('flex w-full cursor-pointer items-center gap-2 rounded-lg border bg-canvas text-ink outline-none transition hover:bg-panel focus-visible:ring-3 focus-visible:ring-accent/15 disabled:cursor-not-allowed disabled:opacity-50', compact ? 'h-[34px] px-2 text-[11px]' : 'h-[38px] px-3 text-xs', open ? 'border-accent' : 'border-line hover:border-line-strong')} disabled={disabled} aria-label={ariaLabel} aria-haspopup="dialog" aria-expanded={open} onClick={() => setOpen(current => !current)}><span className={cn('shrink-0 rounded border-2 border-white/20 shadow-[0_0_0_1px_rgb(0_0_0/.5)]', compact ? 'size-[18px]' : 'size-[19px]')} style={{background:value}}/><code className="min-w-0 flex-1 truncate text-left font-mono text-[10px] text-secondary">{value.toUpperCase()}</code><ChevronDown className={cn('shrink-0 text-muted transition-transform', open && 'rotate-180 text-accent')} size={13}/></button>
    {open && position && createPortal(<div ref={popupRef} className="animate-[control-popover-in_.14s_cubic-bezier(.2,.8,.2,1)] rounded-xl border border-line-strong bg-[rgb(24_27_23/.98)] p-3 text-secondary shadow-dialog backdrop-blur-xl" role="dialog" aria-label={ariaLabel} style={position} onKeyDown={event => {if (event.key === 'Escape') {event.preventDefault(); setOpen(false); triggerRef.current?.focus()}}}>
      ...
      <div className="grid grid-cols-5 gap-2 py-3">{COLOR_PRESETS.map(color => <button type="button" key={color} className={cn('grid size-[30px] min-h-[30px] cursor-pointer place-items-center rounded-lg border border-white/10 text-white outline-none transition hover:scale-105 focus-visible:ring-2 focus-visible:ring-accent', color.toLowerCase() === value.toLowerCase() && 'ring-2 ring-accent ring-offset-2 ring-offset-panel')} aria-label={color} title={color} style={{background:color}} onClick={() => {onChange(color); setDraft(color)}}>{color.toLowerCase() === value.toLowerCase() && <Check className="drop-shadow" size={12}/>}</button>)}</div>
...
export function RangeControl({value, min, max, step = 1, onChange, ariaLabel}: RangeControlProps) {
  ...
  return <input className="mt-2 h-[22px] w-full cursor-pointer appearance-none bg-transparent [&::-moz-range-progress]:h-1 [&::-moz-range-progress]:rounded-full [&::-moz-range-progress]:bg-accent [&::-moz-range-thumb]:size-3.5 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-[3px] [&::-moz-range-thumb]:border-accent-strong [&::-moz-range-thumb]:bg-[#dffbf3] [&::-moz-range-thumb]:shadow-[0_2px_8px_rgb(0_0_0/.53)] [&::-moz-range-track]:h-1 [&::-moz-range-track]:rounded-full [&::-moz-range-track]:bg-line [&::-webkit-slider-runnable-track]:h-1 [&::-webkit-slider-runnable-track]:rounded-full [&::-webkit-slider-runnable-track]:[background:linear-gradient(90deg,var(--color-accent)_0_var(--range-progress),var(--color-line)_var(--range-progress)_100%)] [&::-webkit-slider-thumb]:mt-[-5px] [&::-webkit-slider-thumb]:size-3.5 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border-[3px] [&::-webkit-slider-thumb]:border-accent-strong [&::-webkit-slider-thumb]:bg-[#dffbf3] [&::-webkit-slider-thumb]:shadow-[0_2px_8px_rgb(0_0_0/.53)] hover:[&::-webkit-slider-thumb]:scale-[1.13] hover:[&::-webkit-slider-thumb]:shadow-[0_0_0_4px_rgb(16_211_163/.12),0_2px_8px_rgb(0_0_0/.53)]" type="range" min={min} max={max} step={step} value={value} aria-label={ariaLabel} style={{'--range-progress': `${progress}%`} as CSSProperties} onChange={event => onChange(Number(event.target.value))}/>
}
