...
class TaskManager:
    ...
    def _schedule(self, task_id: str) -> None:
        with self.lock:
            ...
            if task_id in self.cancel_before_start:
                self.cancel_before_start.discard(task_id)
                return
        ...
        task.add_done_callback(lambda _: self._task_finished(task_id))
    ...
    def cancel(self, task_id: str) -> None:
        ...
        # running, allowing a second GPU job to overlap it. The API persists the
        # CANCELLED state; PipelineProcessor observes it cooperatively.
        with self.lock:
            self.cancel_before_start.add(task_id)
            running = self.running.get(task_id)
            worker_started = task_id in self.workers_started
    ...
    def has_started(self, task_id: str) -> bool:
        with self.lock:
            return task_id in self.workers_started

    def is_active(self, task_id: str) -> bool:
        with self.lock:
            running = self.running.get(task_id)
            return task_id in self.pending or bool(running and not running.done())
